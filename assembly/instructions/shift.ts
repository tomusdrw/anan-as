...
export const shar_r_imm_32: InstructionRun = (r, args, registers) => {
  ...
  registers[reg(args.b)] = value >> i64(shift);
  return ok(r);
};
...
export const shar_r_imm_alt_32: InstructionRun = (r, args, registers) => {
  ...
  registers[reg(args.b)] = u32SignExtend(u32(imm >> i64(shift)));
  return ok(r);
};
...
export const shar_r_32: InstructionRun = (r, args, registers) => {
  ...
  registers[reg(args.c)] = u32SignExtend(u32(regValue >> i64(shift)));
  return ok(r);
};
...
export const shar_r: InstructionRun = (r, args, registers) => {
  const shift = u32(registers[reg(args.a)] % u64(MAX_SHIFT_64));
  registers[reg(args.c)] = i64(registers[reg(args.b)]) >> i64(shift);
  return ok(r);
};
