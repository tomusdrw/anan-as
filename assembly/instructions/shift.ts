...
const MAX_SHIFT_64: u32 = 64;
const MAX_SHIFT_32: u32 = 32;
...
export const shar_r_imm_32: InstructionRun = (r, args, registers) => {
  ...
  registers[reg(args.b)] = value >> u64(shift);
  return ok(r);
};
...
export const shar_r_imm_alt_32: InstructionRun = (r, args, registers) => {
  const shift = u64(u32(registers[reg(args.a)] % u64(MAX_SHIFT_32)));
...
export const shlo_r_imm: InstructionRun = (r, args, registers) => {
  const shift = u32(args.c % MAX_SHIFT_64);
  registers[reg(args.b)] = shlo_r_internal(registers[reg(args.a)], u64(shift));
  return ok(r);
};
...
export const shlo_l_imm_alt: InstructionRun = (r, args, registers) => {
  ...
  registers[reg(args.b)] = u32SignExtend(args.c) << u64(shift);
  return ok(r);
};
...
export const shlo_r_imm_alt: InstructionRun = (r, args, registers) => {
  const shift = u32(registers[reg(args.a)] % u64(MAX_SHIFT_64));
  registers[reg(args.b)] = shlo_r_internal(u32SignExtend(args.c), u64(shift));
  return ok(r);
};
...
export const shar_r_imm_alt: InstructionRun = (r, args, registers) => {
  ...
  registers[reg(args.b)] = u32SignExtend(u32(value >> u64(shift)));
  return ok(r);
};
...
export const shar_r_32: InstructionRun = (r, args, registers) => {
  ...
  registers[reg(args.c)] = u32SignExtend(u32(regValue >> u64(shift)));
  return ok(r);
};
...
export const shlo_r: InstructionRun = (r, args, registers) => {
  const shift = u32(registers[reg(args.a)] % u64(MAX_SHIFT_64));
  registers[reg(args.c)] = shlo_r_internal(registers[reg(args.b)], u64(shift));
  return ok(r);
};

// SHAR_R
export const shar_r: InstructionRun = (r, args, registers) => {
  const shift = u64(u32(registers[reg(args.a)] % u64(MAX_SHIFT_64)));
