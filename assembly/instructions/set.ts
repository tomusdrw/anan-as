...
export const set_lt_u_imm: InstructionRun = (r, args, registers) => {
  ...
  registers[reg(args.b)] = u64(cond ? 1 : 0);
  return ok(r);
};
...
export const set_lt_u: InstructionRun = (r, args, registers) => {
  registers[reg(args.c)] = u64(registers[reg(args.b)] < registers[reg(args.a)] ? 1 : 0);
  return ok(r);
};

// SET_LT_S
export const set_lt_s: InstructionRun = (r, args, registers) => {
  registers[reg(args.c)] = u64(i64(registers[reg(args.b)]) < i64(registers[reg(args.a)]) ? 1 : 0);
  return ok(r);
};
